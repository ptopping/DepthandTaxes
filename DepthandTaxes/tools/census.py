...
class CensusLoad(object):
	...
	def pretty(self):
		...
		df = df.melt(id_vars=['STATE', 'DIVISION', 'RACE']).pivot_table(index=['STATE','DIVISION','variable'],columns='RACE',values='value')
		df = df.reset_index().rename(columns={'variable':'YEAR'})
		return df
	...
	def projection(self):
		...
		#Reshape dataframe
		df = df.melt(id_vars=['STATE','DIVISION','YEAR'])
		df = df.reset_index().rename(columns={'variable': 'RACE', 'value': 'POPULATION'})
		df = df.pivot_table(index=['STATE','DIVISION','RACE'], columns='YEAR', values='POPULATION').reset_index()
		
		#Merge future dataframe
		df = df.merge(self.future_df,how='left',left_on='RACE',right_index=True)

		#Calculate future population estimates
		df[2018] = df[2017] * (1+df['2020rate'])**(2018-2017)
		df[2020] = df[2017] * (1+df['2020rate'])**(2020-2017)
		df[2024] = df[2017] * (1+df['2030rate'])**(2024-2017)
		df[2028] = df[2017] * (1+df['2030rate'])**(2028-2017)
		df[2032] = df[2017] * (1+df['2040rate'])**(2032-2017)
		df[2036] = df[2017] * (1+df['2040rate'])**(2036-2017)
		df[2040] = df[2017] * (1+df['2040rate'])**(2040-2017)

		#Reshape dataframe
		df = df[['STATE', 'DIVISION', 'RACE', 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2020, 2024, 2028, 2032, 2036, 2040]]
		df = df.melt(id_vars=['STATE','RACE','DIVISION']).pivot_table(index=['STATE','DIVISION','variable'],columns='RACE',values='value')
		df = df.reset_index().rename(columns={'variable':'YEAR'})
		return df
